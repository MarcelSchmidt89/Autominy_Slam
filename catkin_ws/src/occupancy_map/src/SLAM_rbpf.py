...
def Slam(particle_index):
    ...
    #print("odom"+str(particle_odom)+" "+str(old_odom)+" "+str(new_odom))


    obs = []
    for ob in observations:
        if(min_viewdistance < np.linalg.norm(ob) <= max_viewdistance) and (angle_between_vectors(ob,np.array([1,0])) < 0.55) :
        #print(transform_point(ob, particle_odom[2], np.array([particle_odom[0],particle_odom[1]])))
            obs.append(transform_point(ob, particle_odom[2], np.array([particle_odom[0],particle_odom[1]])))

        
    #if(len(prev_observations[particle_index])>0):
    mat, rot, trans = icp(np.array(obs),observation_estimates[particle_index], max_iterations=100)

    

    odom_T = v2t(particle_odom)
    particle_odom = t2v(np.copy(np.dot(mat, odom_T)))
    particle_odoms[particle_index] = particle_odom

    prev_observations[particle_index] = np.array(obs)

    observation_estimates[particle_index] = np.array(obs)
    ...
    return particle_odoms[particle_index], particle_weights[particle_index],occupancy_grids[particle_index],likelyhood_grids[particle_index],observation_estimates[particle_index], prev_observations[particle_index]
...
def listener():
    ...
    prev_observations = [None]*particles_amount

    indixes = range(particles_amount)

    obs = []
    for ob in observation_records[0]:
        #print(transform_point(ob, particle_odom[2], np.array([particle_odom[0],particle_odom[1]])))
        obs.append(transform_point(ob, odom_records[0][2], np.array([odom_records[0][0],odom_records[0][1]])))
    ...
    for i in range(0,particles_amount):
        ...
        prev_observations[i] = np.array(obs)
    ...
    for i in range(0,len(odom_records)):
        ...
        odom_quat = tf.transformations.quaternion_from_euler(0,0,new_odom[2])
        odom_msg.pose.pose = Pose(Point(new_odom[0],new_odom[1],0),Quaternion(*odom_quat))
